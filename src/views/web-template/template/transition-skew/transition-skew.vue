...
function createNavDots() {
  ...
  for (let i = 0; i < totalPanels; i++) {
    ...
    dot.className = 'tsk-nav-dot' + (i === 0 ? ' tsk-active' : '')
...
function updateUI(index: number) {
  document.querySelectorAll('.tsk-nav-dot').forEach((dot, i) => {
    dot.classList.toggle('tsk-active', i === index)
  })
  const el = document.querySelector('.tsk-page-indicator .tsk-current')
...
function buildTimeline(): gsap.core.Timeline {
  ...
  gsap.set(panels, { opacity: 0, skewX: 0 })
  gsap.set(panels[0], { opacity: 1 })

  for (let i = 0; i < segments; i++) {
    tl.to(panels[i], {
      skewX: -22, opacity: 0, x: '-8%',
      duration: 1, ease: 'power2.inOut'
    }, i)
    tl.fromTo(panels[i + 1],
      { skewX: 22, opacity: 0, x: '8%' },
      { skewX: 0, opacity: 1, x: '0%', duration: 1, ease: 'power2.inOut' },
      i
    )
  }
...
function init() {
  ...
  scrollArea = document.getElementById('tskScrollArea') as HTMLElement
  stage = document.getElementById('tskStage') as HTMLElement
  progressBar = document.getElementById('tskProgressBar')
  navDots = document.getElementById('tskNavDots')
  if (!scrollArea || !stage) return
  panels = gsap.utils.toArray('.tsk-panel') as HTMLElement[]
...
<template>
  <div class="tsk-page">
    <div id="tskProgressBar" class="tsk-progress-bar"></div>
    <nav id="tskNavDots" class="tsk-nav-dots"></nav>
    <div class="tsk-page-indicator">
      <span class="tsk-current">1</span>
      <span class="tsk-total"> / {{ totalPanels }}</span>
    </div>

    <div id="tskScrollArea" class="tsk-scroll-area" :style="{ height: totalPanels * 100 + 'vh' }">
      <div id="tskStage" class="tsk-stage">
        <section class="tsk-panel tsk-panel-0"><div class="tsk-content"><h2>Panel 1</h2></div></section>
        <section class="tsk-panel tsk-panel-1"><div class="tsk-content"><h2>Panel 2</h2></div></section>
        <section class="tsk-panel tsk-panel-2"><div class="tsk-content"><h2>Panel 3</h2></div></section>
        <section class="tsk-panel tsk-panel-3"><div class="tsk-content"><h2>Panel 4</h2></div></section>
        <section class="tsk-panel tsk-panel-4"><div class="tsk-content"><h2>Panel 5</h2></div></section>
        <section class="tsk-panel tsk-panel-5"><div class="tsk-content"><h2>Panel 6</h2></div></section>
        <section class="tsk-panel tsk-panel-6"><div class="tsk-content"><h2>Panel 7</h2></div></section>
      </div>
    </div>
  </div>
</template>
