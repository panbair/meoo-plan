          ...
          <div class="panel-body">
            ...
            <div class="section">
              ...
              <div class="chat-input-row">
                ...
                />
                <button class="chat-send" @click="sendChat" :disabled="!chatInput.trim() || chatLoading">
                  ➤
                </button>
              </div>
            </div>
...
const renderedPlan = computed(() => {
  ...
  try {
    ...
    return planContent.value.replace(/\n/g, '<br>')
  }
})
...
async function handleGenerate() {
  ...
  try {
    ...
    const tplInfo = props.selectedTemplateKeys.length > 0
      ? {
          key: props.selectedTemplateKeys[0],
          label: props.templateLabels[props.selectedTemplateKeys[0]] || props.selectedTemplateKeys[0],
          panelCount: props.modulePositions.length,
    ...
    planContent.value = await aiService.generateOrchestrationPlan({
      ...
      userNotes: props.hasEnterprise
        ? `企业: ${props.enterpriseInfo.name || ''}, 行业: ${props.enterpriseInfo.industry || ''}, 业务: ${props.enterpriseInfo.descriptionription || ''}`
        : undefined
    })
...
function buildLocalPlan(): string {
  const comps = props.selectedComponents
  const tplKeys = props.selectedTemplateKeys
  const panels = props.modulePositions

  const compList = comps.map(c => `- **${c.name}**`).join('\n')
  const panelList = panels.map(p => {
    const pcomps = props.getComponentsByPosition(p.key)
    return `- **${p.icon} ${p.label}**: ${pcomps.length > 0 ? pcomps.map((c: any) => c.name).join(', ') : '(待分配)'}`
  }).join('\n')
...
## 📋 项目概览
- **选择组件**: ${comps.length} 个
- **选择模板**: ${tplKeys.length > 0 ? tplKeys.map(k => props.templateLabels[k] || k).join(', ') : '未选择'}
...
- Canvas 粒子组件限制在移动端 <= 30 个粒子
- 首屏目标 LCP < 2.5s

---
> 📝 本方案由本地模式生成 | ${new Date().toLocaleString('zh-CN')}
> 💡 配置 DeepSeek API 可获得更详细的 AI 定制方案
`
}
  ...
  try {
    if (!isAIAvailable()) {
      chatMessages.value.push({
        role: 'assistant',
        content: '⚠️ 未配置 AI API。请配置 DeepSeek API Key 后重试。'
      })
    ...
    const compList = props.selectedComponents.map(c => c.name).join(', ')
    const tplList = props.selectedTemplateKeys.map(k => props.templateLabels[k] || k).join(', ')

    const response = await aiService.chat({
      messages: [
        {
          role: 'system',
          content: `你是组件选择顾问。用户正在为企业网站挑选 GSAP 动画组件。当前选择: 组件=[${compList}], 模板=[${tplList}]。请用简洁专业的中文回答，不超过200字。`
        },
    ...
    if (response.success && response.data?.content) {
      chatMessages.value.push({ role: 'assistant', content: response.data.content })
    } else {
      chatMessages.value.push({ role: 'assistant', content: '抱歉，请求失败，请重试。' })
    }
  } catch (e: any) {
...
function downloadPlan() {
  ...
  a.href = url
  a.download = `website-plan-${Date.now()}.md`
...
.chat-msg {
  ...
  &.msg-user {
    ...
    .msg-text { background: rgba(99,102,241,0.1); color: #a5b4fc; }
  }
}
...
.chat-send {
  ...
  &:hover:not(:disabled) { background: rgba(99,102,241,0.4); color: white; }
...
.markdown-content {
  ...
  :deep(table) { width: 100%; border-collapse: collapse; font-size: 0.72rem; }
  :deep(th), :deep(td) { padding: 4px 8px; border: 1px solid rgba(255,255,255,0.06); text-align: left; }
  :deep(th) { background: rgba(255,255,255,0.03); color: #e2e8f0; }
}
...
.slide-leave-active {
  ...
  .ai-panel { transition: transform 0.3s cubic-bezier(0.4, 0, 0.2, 1); }
}
