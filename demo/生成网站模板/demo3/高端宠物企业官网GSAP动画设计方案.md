...
#backToTop {
  ...
  box-shadow: 0 8px 30px rgba(59, 130, 246, 0.5), 0 0 50px rgba(59, 130, 246, 0.3);
}
...
function initBackToTop() {
  ...
  btn.addEventListener('click', () => {
    ...
    gsap.to(btn, { rotation: 360, duration: 1, ease: 'power2.out' });
  });
  
  // 悬停效果
  btn.addEventListener('mouseenter', () => {
    gsap.to(btn, { scale: 1.1, boxShadow: '0 0 30px rgba(59, 130, 246, 0.5)', duration: 0.3 });
  });
  btn.addEventListener('mouseleave', () => {
    gsap.to(btn, { scale: 1, boxShadow: '0 8px 30px rgba(59, 130, 246, 0.5), 0 0 50px rgba(59, 130, 246, 0.3)', duration: 0.3 });
  });
}

// 在 initAnimations 中调用
...
**关键要点：**
- CSS 必须放在全局样式中，不能放在 `@media` 媒体查询内部
- 使用 `id="backToTop"` 配合 `#backToTop` 选择器
- 添加 `.visible` 类控制显示/隐藏，而不是直接操作 opacity/visibility
- 需要引入 `ScrollToPlugin` 并注册
...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    @media (max-width: 768px) {
      #backToTop { bottom: 20px; right: 20px; width: 48px; height: 48px; }
    }
  </style>
</head>
<body>
